...
namespace _1911064890_HaNguyenGiaMInh
{
    public partial class frmCalculator : Form
    {
        ...
        {
            InitializeComponent();
            this.KeyPreview = true; // Cho phép Form nhận phím trước các control
        ...
        private void frmCalculator_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (char.IsDigit(keyChar) || "+-*/().".Contains(keyChar))
            {
                if (justEvaluated)
                {
                    // Nếu vừa tính xong và ký tự đầu là toán tử → tiếp tục từ kết quả
                    if ("+-*/".Contains(keyChar))
                    {
                        inputExpression = lastResult.ToString() + keyChar;
                    }
                    else
                    {
                        inputExpression = keyChar.ToString(); // Nếu là số → bắt đầu biểu thức mới
                    }
                    justEvaluated = false;
                }
                else
                {
                    inputExpression += keyChar;
                }

                lbl_NoiDung.Text = inputExpression;
            }
            // Nhấn Enter để tính
            else if (keyChar == (char)Keys.Enter)
            {
                try
                {
                    var dt = new DataTable();
                    var resultObj = dt.Compute(inputExpression, "");
                    double result = Convert.ToDouble(resultObj);

                    lbl_NoiDung.Text = inputExpression + " = " + result.ToString();
                    lastResult = result;
                    justEvaluated = true;
                }
                catch
                {
                    lbl_NoiDung.Text = "Biểu thức sai!";
                    inputExpression = "";
                    justEvaluated = false;
                }
            }
            ...
            else if (keyChar == (char)Keys.Back)
            {
                if (inputExpression.Length > 0)
                {
                    ...
                    lbl_NoiDung.Text = inputExpression == "" ? "0" : inputExpression;
                }
            }
        }
